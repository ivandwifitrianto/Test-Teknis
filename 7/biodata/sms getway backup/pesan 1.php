...
<h1> Refresh halaman ( F5 ) untuk menjalankan sms gateway<h1>
...
while ($data = mysql_fetch_array($hasil))
{
  ...
  if ($pecah[0] == "PELANGGARAN")
  {
     ...
     // cari pelanggaran berdasarkan NIS
     $query2 = "SELECT Kode_pelanggaran FROM pelanggaran_siswa WHERE NIS_siswa = '$pel'";
     $hasil2 = mysql_query($query2);

	 $query3 = "SELECT DATE_FORMAT(Tanggal_pelanggaran_siswa, '%d-%m-%Y')as Tanggal_pelanggaran_siswa FROM pelanggaran_siswa WHERE NIS_siswa = '$pel'";
     $hasil3 = mysql_query($query3);
	 
	
     // cek bila data nilai tidak ditemukan
     if (mysql_num_rows($hasil2 )==0) $reply = "NIS tidak ada.Sillahkan cek kembali NIS siswa";
     else
     {
        // bila pelangggaran ditemukan
       
	    $data2 = mysql_fetch_array($hasil2);
        $nilai = $data2['Kode_pelanggaran'];
		$data3 = mysql_fetch_array($hasil3);
        $nilai1 = $data3['Tanggal_pelanggaran_siswa'];
        $reply = "Pelanggaran : ".$nilai. " . Pada tanggal : ".$nilai1;
